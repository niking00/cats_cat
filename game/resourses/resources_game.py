...
from resourses.const import PERSONAGE, UNITS, OBJECTS, DIRECTIONS,\
                  TIME_CLICK, MUSIC, LEVELS, DIRECTORY
from resourses.resources import Algorithms
from resourses.level import create_level, new_frame
...
class Game:
    ...
    def new_list_level_cat(self):
        ...
        while not self.win and not self.kill:
            while key not in DIRECTIONS + ['esc']:
                key = keyboard.read_key()
            if key == 'esc':
                break
            ...
            if object == OBJECTS[0]:
                ...
                break
        if not self.kill and not self.win and not key == 'esc':
    ...
    def writing_DB(self):
        global old_time
        print('Хорошая работа, Оле... Ой, а как тебя зовут, юзер?\n'
              'Моё имя (оно будет зваписано в турнирной таблице) : ', end='')
        name = ''
        data = int(time.time() - old_time)
        while not name:
            name = input()
        os.system('cls')
        print('Отлично, хорошая работа, ' + name + '!',
              'Ваше имя будет увековечено в турнирной таблице\n'
              'или перезаписано, если вы улучшили свой результат!')
        with open(DIRECTORY + "/resourses/results.json", "r", encoding="utf-8") as file:
            js = json.load(file)
            js[str(data)] = name
        with open(DIRECTORY + "/resourses/results.json", "w", encoding="utf-8") as file:
            json.dump(js, file)
        time.sleep(4)